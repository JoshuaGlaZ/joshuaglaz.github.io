...
subtitle: "Python constructs fully implemented"
...
Continue from enhancing the ast construction based from the `code_component`, 
this week is mostly used for adding the Python construct that previous week has not been implemented yet. 

Beside for the child node (e.g. Literals, Imports), the definition for each node is going to by more dynamic. 
The connecting the node with another by using compositions to make relationship where the `part_id` in `composition` 
is the same as the id of `code_component`, as in `construct_node`,  
whereas previous `construct_node` the attribute such as value, elts, name, and other is mostly using the name of `code_component`,
either by manually spliting, extract the call function, or just use the value as it is and instatiate under ast.Constant
...
Addtinionally, I add another atribute for the nodes, which is the label. node.label is going to be the label of the node visualization graph. These label is mainly from the name of `code_component`, appart from `function_def` & `class_def`.

### Querying the labels for def nodes
The complicated label for the `function_def` & `class_def` will be query from the database. Below query extracts the IDs of code components that are either function definitions (function_def) or class definitions (class_def) for a specific trial. The resulting IDs are used as a subquery to be utilized in the next query.
...
Then, details of the `code_component` is needed. Below query fetches details of code components related to the previously identified function and class definitions for the same trial. The details is the name, type, whole_id, and position fields from the CodeComponent and Composition tables. We join these tables on matching id and trial_id fields. We filter the results to exclude components of type syntax and to include only those whose whole_id is in the list of IDs from our previous query.
...
The query results then finally converted into a list of dictionaries for easier manipulation and readability.
...
As mentioned before, the relationship for the `code_component` & `composition` is more dynamic. With python constructs coverage in noworkflow, the attribute of the nodes is more defined. Each block handles how to assign or append the attribute of parent node to current iteration node based on the composition type.
...
The label for the ast is displayed as mention before. The sample of the output label will be
> Name <br>
> x
...
The labels now is more clear for node that has multiple nodes, like if, for, function_def, etc.
